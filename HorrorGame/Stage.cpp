	...
	,position(VGet(30.0f,0,50.0f))
{
	...
	//プレイヤーの回転
	MV1SetRotationXYZ(StageHandle, VGet(0.0f, 0 + DX_PI_F, 0.0f));
...
/// </summary>
VECTOR Stage::CheckCollision(PlayerBase& player, const VECTOR& moveVector)
{
	...
	// HACK: ステージポリゴンが複数ある場合、ここが繰り返し処理になる
	{
		...
		// ( 検出する範囲は移動距離も考慮する )
		auto hitDim = MV1CollCheck_Sphere(StageHandle, -1, oldPos, DefaultSize + VSize(moveVector));
...
/// 当たり判定をして、補正した移動先のポジションを返す
/// </summary>
VECTOR Stage::CheckEnemyCollision(EnemyBase& enemy, const VECTOR& moveVector)
{
	VECTOR oldPos = enemy.GetPosition();						// 移動前の座標	
	VECTOR nextPos = VAdd(oldPos, moveVector);		// 移動後の座標	
	...
	// HACK: ステージポリゴンが複数ある場合、ここが繰り返し処理になる
	{
		...
		// 壁ポリゴンとの当たりをチェックし、移動ベクトルを補正する
		nextPos = CheckEnemyHitWithWall(enemy, nextPos);

		// 床ポリゴンとの当たりをチェックし、移動ベクトルを補正する
		nextPos = CheckEnemyHitWithFloor(enemy, nextPos);
...
/// </summary>
VECTOR Stage::CheckHitWithWall(PlayerBase& player, const VECTOR& checkPosition)
{
...
/// </summary>
VECTOR Stage::CheckHitWithFloor(PlayerBase& player, const VECTOR& checkPosition)
{
	VECTOR fixedPos = checkPosition;
...
/// 壁ポリゴンとの当たりをチェックし、補正すべき移動ベクトルを返す
/// </summary>
VECTOR Stage::CheckEnemyHitWithWall(EnemyBase& enemymove, const VECTOR& checkPosition)
{
	VECTOR fixedPos = checkPosition;
...
/// 床ポリゴンとの当たりをチェックし、補正すべき移動ベクトルを返す
/// </summary>
VECTOR Stage::CheckEnemyHitWithFloor(EnemyBase& enemymove, const VECTOR& checkPosition)
{
	VECTOR fixedPos = checkPosition;
